...
// Sorted by threat level: hypersonic > ballistic (by speed) > cruise > drone
...
const TEXT = {
  ...
  type: { en: "Type", es: "Tipo", he: "סוג" },
  name: { en: "Name", es: "Nombre", he: "שם" },
  attacks: { en: "Attacks", es: "Ataques", he: "מתקפות" },
  fact: { en: "Fun fact", es: "Dato", he: "עובדה מעניינת" },
  used_in: { en: "Used in", es: "Usado en", he: "שימוש ב" },
  cards_view: { en: "Cards", es: "Tarjetas", he: "כרטיסים" },
  table_view: { en: "Table", es: "Tabla", he: "טבלה" },
  disclaimer: {
...
function CardsView({ lang }: { lang: Lang }) {
  return (
...
function TableView({ lang }: { lang: Lang }) {
  return (
      ...
      <table className="arsenal-table">
        <thead>
          <tr>
            ...
            <th>{TEXT.range[lang]}</th>
            <th>{TEXT.speed[lang]}</th>
            <th>{TEXT.warhead[lang]}</th>
            <th>{TEXT.attacks[lang]}</th>
          </tr>
        </thead>
        <tbody>
          {WEAPONS.map((w) => (
            <tr key={w.name}>
              <td className="table-weapon-name">{w.name}</td>
              <td>
                <span className="weapon-type-badge table-badge" style={{ background: TYPE_COLORS[w.type] }}>
                  {w.typeLabel[lang]}
                </span>
              </td>
              <td>{w.range}</td>
  ...
  return (
    <div className="panel">
      <div className="arsenal-header">
        ...
        <div className="arsenal-view-toggle">
          <button
            className={`view-btn ${view === "cards" ? "active" : ""}`}
            onClick={() => setView("cards")}
          >{TEXT.cards_view[lang]}</button>
          <button
            className={`view-btn ${view === "table" ? "active" : ""}`}
            onClick={() => setView("table")}
          >{TEXT.table_view[lang]}</button>
        </div>
      </div>

      {view === "cards" ? <CardsView lang={lang} /> : <TableView lang={lang} />}
